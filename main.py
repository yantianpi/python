...
# 导入工具函数包
from common import webHelper, logHelper
...
def validate():
    ...
    if path_info in ['/favicon.ico', '/', '/api/verify/', '/index/']:
        return

    ### 记录客户端提交的参数 ###
    # 获取当前访问url路径与ip
    request_log = 'url:' + path_info + ' ip:' + webHelper.get_ip()
    try:
        # 添加json方式提交的参数
        if request.json:
...
if __name__ == '__main__':
    ...
    run(app=app_argv, host='127.0.0.1', port=9090, debug=True, reloader=True)
else:
