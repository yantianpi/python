...
from common import datetimeHelper, randomHelper, logHelper
...
def callback():
    ...
    try:
        ...
        # 给上传的文件重命名，默认上传的是图片
        if ext and ext != '':
            file_name = datetimeHelper.to_number() + randomHelper.get_string(5) + ext
        else:
            file_name = datetimeHelper.to_number() + randomHelper.get_string(5) + '.jpg'
        upload.filename = file_name

        # 设置文件存储的相对路径
        filepath = '/upload/' + datetimeHelper.to_number('%Y%m%d') + '/'
        # 组合成服务器端存储绝对路径
        ...
        logHelper.error('上传失败：' + str(e.args))
